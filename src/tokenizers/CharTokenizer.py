...
class CharTokenizer():
    ...
    def convert_tokens_to_ids(self, tokens: List[str]) -> List[int]:
        encodings: List[int] = []
        for char in tokens:
            if char in self.token2id:  # Check if the character is in the tokenizer's vocabulary
                encodings.append(self.token2id[char])
            else:
                encodings.append(self.token2id['[UNK]'])  # Use the unknown token for unrecognized characters
        return encodings
