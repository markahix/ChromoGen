...
def fail_safe(func: Callable[[Chem.rdchem.Mol], float], mol: Chem.rdchem.Mol) -> float:
    try:
        res = func(mol)
        if res is None:
            print(f"Warning: Function returned None for molecule {Chem.MolToSmiles(mol)}")
        return res
    except Exception as e:
        print(f"Error with molecule {Chem.MolToSmiles(mol)}: {e}")
        return None
...
def calc_set_stat(mol_set: List[Chem.rdchem.Mol],
    ...
    print(f"Calculating stats for {len(mol_set)} molecules")
    ...
    if None in batch_rewards:
        ...
        print("Number of None values:", batch_rewards.count(None))
...
def get_stats(train_set: Dataset,
    ...
    # Calculate the number of rings for each molecule
    num_rings = [calc_num_rings(mol) for mol in generated_molecules]
    ...
    print(f"Number of valid_generated_smiles: {len(valid_generated_smiles)}")
    print(f"Reward Function: {str(reward_fn)}")                                                         
    ...
    data = {**{'Smiles': valid_generated_smiles},
            ...
            **{"Ring Count": num_rings}  # Add the ring count data
            }

    for k, v in data.items():
        print(f'{k=} {len(v)=}')

    # Create DataFrame and sort by Ring Count
    df = pd.DataFrame(data)
    df.sort_values(by='Ring Count', ascending=False, inplace=True)
